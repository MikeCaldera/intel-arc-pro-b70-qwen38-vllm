...
| Qwen3.8-Flash-Next (llama.cpp SYCL, dual B70 **C1**) | llama.cpp `9723942adc518b43c4b95dc4dce6906903eb5e09` (not a vLLM image) | `patches/llamacpp-sycl/flashnext-arch-overlay.patch`, then `sycl-fused-mmvq-iq3s-iq4nl.patch`. Optional grouping: `sycl-mmid-gpu-group-20260901.patch` (`GGML_SYCL_DISABLE_MMID_GPU_GROUP=1`). Pick **one** `GGML_SYCL_F16` binary. [Recipe](qwen38-flash-next/QWEN38-FLASH-NEXT-LLAMACPP.md). | All vLLM Qwen MTP / FP8 / Nemotron / Ornith patches. Do not mix fused-FP32 decode with fused-F16 prefill as one recipe. `c8`/`c16`/`c128` are context, not concurrency. |

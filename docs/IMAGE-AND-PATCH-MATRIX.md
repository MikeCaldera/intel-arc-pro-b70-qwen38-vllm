...
| Ornith-1.5-35B-A3B | `…f01e24f6c7ff…` (same nightly as Qwen3.8) | `patch_mtp_boundary.py` for exact max-len MTP. `patch_mtp_nightly.py` already-in-image if `dynamic` excludes `mtp`. **Default DraftINT4 S+M1** (`B70_DRAFT_LMHEAD_INT4=1`, `B70_DRAFT_MTP_INT4=1`): launcher `DRAFT_INT4=1`. Combined 230 W LMX `tokSOut` 108.4 / `tokSPrefill` 9073 (`cmt2tdx5q0hy0mv01koh4xwpw`). Weights: local MixedCal-v2 experts-only GPTQ ([MIXEDCAL-V2.md](ornith15-35a3/MIXEDCAL-V2.md); RTN 24.76% → 10.37%). | Nemotron grouped-topk / SSU. GDN mixed-split v5 on C1. Do not expect the v0.21 native-int4moe 204.6 path on this image. |

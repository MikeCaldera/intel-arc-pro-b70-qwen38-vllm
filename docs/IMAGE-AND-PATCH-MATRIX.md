...
| Ornith-1.5-35B-A3B | `…f01e24f6c7ff…` (same nightly as Qwen3.8) | `patch_mtp_boundary.py` for exact max-len MTP. `patch_mtp_nightly.py` already-in-image if `dynamic` excludes `mtp`. Optional DraftINT4 S+M1 (`B70_DRAFT_LMHEAD_INT4=1`, `B70_DRAFT_MTP_INT4=1`): MTP1 n=5 106.27 vs 96.43 BF16 draft. Weights: local MixedCal-v2 experts-only GPTQ ([MIXEDCAL-V2.md](ornith15-35a3/MIXEDCAL-V2.md); RTN 24.76% → 10.37%). | Nemotron grouped-topk / SSU. GDN mixed-split v5 on C1. Do not expect the v0.21 native-int4moe 204.6 path on this image. |

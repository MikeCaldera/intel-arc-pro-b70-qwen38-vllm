...
& $docker run --rm --device /dev/dxg -v /usr/lib/wsl/lib:/usr/lib/wsl/lib:ro -v /usr/lib/wsl/drivers:/usr/lib/wsl/drivers:ro -e LD_LIBRARY_PATH=/usr/lib/wsl/lib:/tmp/ucx_install/lib:/opt/venv/lib:/usr/local/lib --entrypoint python qwen38-b70-docker:2026.08.18 -c "import torch; assert torch.xpu.is_available(); x=torch.arange(1048576,device='xpu'); y=(x*2+1).sum(); torch.xpu.synchronize(); print('B70:',torch.xpu.get_device_name(0)); print('XPU allocation and compute probe: OK')"
